...
public class CinemaList {
    ...
    private JButton save, add, delete, upload, print, edit, saveXML, loadXML, savePDF, saveHTML;
    ...
    public void show() {
        ...
        // Добавление кнопок для сохранения в PDF и HTML форматы
        savePDF = new JButton(new ImageIcon("src/img/pdf_save.png"));
        savePDF.setToolTipText("Сохранить данные в PDF");
        toolBar.add(savePDF);
        ...
        saveHTML = new JButton(new ImageIcon("src/img/html_save.png"));
        saveHTML.setToolTipText("Сохранить данные в HTML");
        ...
        // Запуск потоков
        new tthread(1, modelMovies, tableMovies).start();
        new tthread(2, modelMovies, tableMovies).start();
        new tthread(3, modelMovies, tableMovies).start();
    ...
    private void saveToPDF() {
        FileDialog savePDFDialog = new FileDialog(cinemaList, "Сохранение данных в PDF", FileDialog.SAVE);
        savePDFDialog.setFile("*.pdf");
        savePDFDialog.setVisible(true);

        String directory = savePDFDialog.getDirectory();
        String filename = savePDFDialog.getFile();
        if (directory == null || filename == null) return; // Пользователь нажал "Отмена"
        ...
        try {
            bfComic = BaseFont.createFont("src/fonts/Arial.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
        ...
        t.addCell(new PdfPCell(new Phrase("Год", font1)));
        t.addCell(new PdfPCell(new Phrase("Фильм", font1)));
        t.addCell(new PdfPCell(new Phrase("Режиссёр", font1)));
        t.addCell(new PdfPCell(new Phrase("Жанр", font1)));

        for (int i = 0; i < modelMovies.getRowCount(); i++) {
            t.addCell(new Phrase((String) modelMovies.getValueAt(i, 0), font1));
            t.addCell(new Phrase((String) modelMovies.getValueAt(i, 1), font1));
            t.addCell(new Phrase((String) modelMovies.getValueAt(i, 2), font1));
            t.addCell(new Phrase((String) modelMovies.getValueAt(i, 3), font1));
        }
        ...
        document.close();
        JOptionPane.showMessageDialog(cinemaList, "Данные успешно сохранены в PDF.");
    }

    private void saveToHTML() {
        FileDialog saveHTMLDialog = new FileDialog(cinemaList, "Сохранение данных в HTML", FileDialog.SAVE);
        saveHTMLDialog.setFile("*.html");
        saveHTMLDialog.setVisible(true);

        String directory = saveHTMLDialog.getDirectory();
        String filename = saveHTMLDialog.getFile();
        if (directory == null || filename == null) return; // Пользователь нажал "Отмена"
        ...
        pw.println("<TABLE BORDER><TR><TH>Год<TH>Фильм<TH>Режиссёр<TH>Жанр</TR>");
        for (int i = 0; i < modelMovies.getRowCount(); i++) {
            pw.println("<TR><TD>" + (String) modelMovies.getValueAt(i, 0)
                    + "<TD>" + (String) modelMovies.getValueAt(i, 1)
                    + "<TD>" + (String) modelMovies.getValueAt(i, 2)
                    + "<TD>" + (String) modelMovies.getValueAt(i, 3));
        }
        pw.println("</TABLE>");
        pw.close();
        JOptionPane.showMessageDialog(cinemaList, "Данные успешно сохранены в HTML.");
    }
...
class tthread extends Thread {
    ...
    public tthread(int i, DefaultTableModel tableModel, JTable table) {
        type = i;
    ...
    public void run() {
        if (type == 1) {
            synchronized (shared) {
                ...
                try {
                    xmlOpen q = new xmlOpen("Открыть данные XML", tableModel, table);
                } catch (Exception e1) {
                    e1.printStackTrace();
                }
            }
        ...
        if (type == 2) {
            synchronized (shared) {
                ...
                tableModel.addRow(new String[]{"2007", "Зодиак", "Дэвид Финчер", "Детектив"});
        ...
        if (type == 3) {
            synchronized (shared) {
                ...
                pdfSave qwe = new pdfSave(tableModel);
                htmlSave qwe2 = new htmlSave(tableModel);
            }
        }
    }
}