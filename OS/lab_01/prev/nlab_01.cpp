...
void print_sys_flags(DWORD flags){
    ...
    if (flags & FILE_CASE_SENSITIVE_SEARCH) cout << "- File sensetive tu uppercase and lowercase" << endl;
    if (flags & FILE_CASE_PRESERVED_NAMES) cout << "- Retained registry of file names is supported" << endl;
    if (flags & FILE_UNICODE_ON_DISK) cout << "- Unicode supported" << endl;
    if (flags & FILE_PERSISTENT_ACLS) cout << "- ACL access supported" << endl;
    if (flags & FILE_FILE_COMPRESSION) cout << "- File can be compressed" << endl;
    if (flags & FILE_VOLUME_QUOTAS) cout << "- Disk quotas are supported on the specified volume" << endl;
    if (flags & FILE_SUPPORTS_SPARSE_FILES) cout << "- Sparse files are supported on the volume" << endl;
    if (flags & FILE_SUPPORTS_REPARSE_POINTS) cout << "- Reparse points are supported on the specified volume" << endl;
    if (flags & FILE_SUPPORTS_REMOTE_STORAGE) cout << "- Remote storage is supported by the file system" << endl;
    if (flags & FILE_RETURNS_CLEANUP_RESULT_INFO) cout << "- File system returns cleanup result info on successful cleanup" << endl;
    if (flags & FILE_SUPPORTS_POSIX_UNLINK_RENAME) cout << "- POSIX-style unlink and rename operations are supported" << endl;
    if (flags & FILE_VOLUME_IS_COMPRESSED) cout << "- The specified volume is a compressed volume" << endl;
    if (flags & FILE_SUPPORTS_OBJECT_IDS) cout << "- Object identifiers are supported on the specified volume" << endl;
    if (flags & FILE_SUPPORTS_ENCRYPTION) cout << "- Encrypted file system (EFS) is supported on the specified volume" << endl;
    if (flags & FILE_NAMED_STREAMS) cout << "- Named streams are supported on the specified volume" << endl;
    if (flags & FILE_READ_ONLY_VOLUME) cout << "- The specified volume is read-only" << endl;
    if (flags & FILE_SEQUENTIAL_WRITE_ONCE) cout << "- Sequential write-once is supported on the specified volume" << endl;
    if (flags & FILE_SUPPORTS_TRANSACTIONS) cout << "- Transactions are supported on the specified volume" << endl;
    if (flags & FILE_SUPPORTS_HARD_LINKS) cout << "- Hard links are supported on the specified volume" << endl;
    if (flags & FILE_SUPPORTS_EXTENDED_ATTRIBUTES) cout << "- Extended attributes are supported on the specified volume" << endl;
    if (flags & FILE_SUPPORTS_OPEN_BY_FILE_ID) cout << "- Opening by FileID is supported by the file system" << endl;
    if (flags & FILE_SUPPORTS_USN_JOURNAL) cout << "- Update Sequence Number (USN) journaling is supported on the specified volume" << endl;
    if (flags & FILE_SUPPORTS_INTEGRITY_STREAMS) cout << "- Integrity streams are supported by the file system" << endl;
    if (flags & FILE_SUPPORTS_BLOCK_REFCOUNTING) cout << "- Logical cluster sharing between files on the same volume is supported" << endl;
    if (flags & FILE_SUPPORTS_SPARSE_VDL) cout << "- Sparse valid data length (VDL) tracking is supported by the file system" << endl;
    if (flags & FILE_DAX_VOLUME) cout << "- The specified volume is a Direct Access (DAX) volume" << endl;
    if (flags & FILE_SUPPORTS_GHOSTING) cout << "- Ghosting is supported by the file system" << endl;
}
